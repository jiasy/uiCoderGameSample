...
function c_block_glassBlock:init(initDict_)
    ...
    if self.type then--直接放置到场景中的，没附过值
        local _picName = self.main:getBlockPicNameByType(self.type)
        local _texture = cc.Director:getInstance():getTextureCache():addImage(_picName)
        self.blockPic:setTexture(_texture)
    end

    self:gtp(math.random(1,self.totalFrames))
end

--ui stateChange-------------------------------------
function c_block_glassBlock:updateF(type_)
    --Logic here,then change state.
    c_block_glassBlock.super.updateF(self,type_)
end
